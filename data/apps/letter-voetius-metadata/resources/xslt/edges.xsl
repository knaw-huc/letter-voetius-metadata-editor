...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="main">
        ...
        <xsl:for-each select="collection(concat($recs,'?match=record-\d+\.xml&amp;on-error=warning'))">
            ...
            <xsl:text expand-text="yes">{/cmd:CMD/cmd:Header/cmd:MdSelfLink/replace(.,'unl://','')},{//*:Sender//@cmd:valueConceptLink/replace(.,'.*/person/(\d+)','$1')},{//*:Receiver//@cmd:valueConceptLink/replace(.,'.*/person/(\d+)','$1')}{$NL}</xsl:text>
        </xsl:for-each>
    </xsl:template>
